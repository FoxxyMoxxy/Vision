...
import vision.models.low_level_evidences.evidence
import vision.models.core.infer
import vision.models.low_level_evidences.infer
from vision.models.low_level_evidences.evidence import Keywords
from vision.models.low_level_evidences.utils import gather_calls
...
def _okay(js, ast, predictor):
    ...
    apicalls = list(set(chain.from_iterable(
        [vision.models.low_level_evidences.evidence.APICalls.from_call(call) for call in calls])))
    types = list(set(chain.from_iterable(
        [vision.models.low_level_evidences.evidence.Types.from_call(call) for call in calls])))
    keywords = list(set(chain.from_iterable(
        [vision.models.low_level_evidences.evidence.Keywords.from_call(call) for call in calls])))
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        if model_type == 'core':
            model = vision.models.core.infer.BayesianPredictor
        elif model_type == 'lle':
            model = vision.models.low_level_evidences.infer.BayesianPredictor
        else:
        ...
        _shutdown()  # we don't shut down flask directly, but if for some reason it ever stops go ahead and stop Vision
