...
Config options should be given as a JSON file:
{
  ...
  "vision_git_hash": "master",
  "vision_patch_file": "/location/of/patch/file (if any, set to null otherwise)",
  "vision_config": {
    "model": "core",
    "lda_files": [
...
def start_training(ssh, config):
    model_type = config.vision_config['model']
    if model_type == 'core':
        model_dir = '~/vision/src/main/python/vision/models/core'
        # additional setup of LDA files
        for i, lda_file in enumerate(config.vision_config['lda_files']):
        ...
        model_dir = '~/vision/src/main/python/vision/models/low_level_evidences'
    else:
        raise ValueError('Invalid model type in config: ' + model_type)
    exec_command_blocking(ssh, """
            (export LD_LIBRARY_PATH=/usr/local/cuda-8.0/lib64:$LD_LIBRARY_PATH; \
            export LD_LIBRARY_PATH=~/cuda/lib64:$LD_LIBRARY_PATH; \
            export PYTHONPATH=~/vision/src/main/python; \
...
def automate_train(config):
    ...
    with message('Checking out Vision at {}'.format(config.vision_git_hash)):
        exec_command_blocking(ssh, '(cd vision; git pull)'.format(config.vision_git_hash))
        exec_command_blocking(ssh, '(cd vision; git checkout {})'.format(config.vision_git_hash))

    sftp = ssh.open_sftp()
    if config.vision_patch_file is not None:
        with message('Applying patch file {}'.format(config.vision_patch_file)):
            sftp.put(config.vision_patch_file, 'vision/patch')
            exec_command_blocking(ssh, '(cd vision; git apply patch)')

    with message('Configuring Vision for training'):
        tmpfile = '.config-{}.json'.format(time.time())
        with open(tmpfile, 'w') as f:
            json.dump(config.vision_config, fp=f, indent=2)
