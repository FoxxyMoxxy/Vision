...
for ev in $evidence; do
    if [ ! -f DATA-$ev.json ]; then
        python3 -u $BUILD_DIR/python/vision/test/ast_quality_perf_test.py DATA-testing.json \
...
for ev in $all; do
    if [ ! -f DATA-$ev.json ]; then
        python3 -u $BUILD_DIR/python/vision/test/ast_quality_perf_test.py DATA-testing-$ev.json \
            --save $BUILD_DIR/resources/model \
            --evidence all \
            --output_file DATA-$ev.json 2>&1 &
    else
